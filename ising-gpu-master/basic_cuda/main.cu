...
int runc(float alpha, float t, float t_end, float step, char* filename, int N) {
    ...
    while (t < t_end) {
        ...
        for (int i = 0; i < N_EQUILIBRIUM+N_AVERAGE; i++) {
            ...
            flip_spins<<<BL, T>>>(BLACK, J0, J1, J2, t, N, matrix, randomMatrix, L*N);
            flip_spins2<<<BL, T>>>(BLACK, J0, J1, J2, t, N, matrix, randomMatrix, L*N);
            cudaDeviceSynchronize();
            reinitialize_random_matrix(N, randomMatrix);

            cudaMemcpy(d_randomMatrix, randomMatrix, bytes2, cudaMemcpyHostToDevice);
            flip_spins<<<BL, T>>>(WHITE, J0, J1, J2, t, N, matrix, randomMatrix, L*N);
            flip_spins2<<<BL, T>>>(WHITE, J0, J1, J2, t, N, matrix, randomMatrix, L*N);
            cudaDeviceSynchronize();
            reinitialize_random_matrix(N, randomMatrix);

            cudaMemcpy(d_randomMatrix, randomMatrix, bytes2, cudaMemcpyHostToDevice);
            flip_spins<<<BL, T>>>(GREEN, J0, J1, J2, t, N, matrix, randomMatrix, L*N);
            flip_spins2<<<BL, T>>>(GREEN, J0, J1, J2, t, N, matrix, randomMatrix, L*N);
            cudaDeviceSynchronize();
            reinitialize_random_matrix(N, randomMatrix);
